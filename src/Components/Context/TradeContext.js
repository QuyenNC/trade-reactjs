...
export function TradeProvider({ children }){
  ...
  const showModal = (item) => { // open modal
    return () =>{
      setItemTrade(item);
  ...
  const handleOk = () => {
      if(inputs.addPost === "" || inputs.selectPost === "" ){
        ...
        axios.post(url, itemTrade, config)
          .then(res => {
  ...
  const handleChange = (e) => {//onChange input yêu cầu trade
    setInputs({ ...inputs, [e.target.name]: e.target.value });
  }
  const handleCancel = () => {//close modal
    setIsModalVisible(false);
  };
    //end create trade
