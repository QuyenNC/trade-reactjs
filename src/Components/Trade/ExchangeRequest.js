            ...
            return (
              <tradeContext.Consumer>
                {({onPutStatusExchangeTrade}) => (
                  <Space size="middle">
                  <Button type="primary" onClick={onPutStatusExchangeTrade(record._id, 'đồng ý')}>
                    Đồng ý
                  </Button>
                  <Button type="primary" onClick={onPutStatusExchangeTrade(record._id, 'từ chối')}>
                    Từ chối
                  </Button>
                </Space>
                )}
          ...
          case 'đồng ý':
            return (
              <tradeContext.Consumer>
                {({onPutStatusExchangeTrade}) => (
                  <Space size="middle">
                    <Button type="primary"  onClick={onPutStatusExchangeTrade(record._id, 'đang chuyển hàng')}>
                      Đã gửi hàng
                    </Button>
                  </Space>
                )}
          ...
          case 'đang chuyển hàng':
            return (
              <tradeContext.Consumer>
                {({onPutStatusExchangeTrade}) => (
                   <Space size="middle">
                    <Button type="primary" onClick={onPutStatusExchangeTrade(record._id, 'đã nhận được hàng')}>
                      Đã nhận được hàng
                    </Button>
                  </Space>
                )}
        ...
        <div className="ExchangeRequest">
            <div className="wrap">
                ...
                <div className="ex-table">
                  <tradeContext.Consumer>
                    {({tradeRequest}) => (
                      <Table  size='middle' columns={columns} pagination={{ pageSize: 7}} dataSource={tradeRequest.filter(item => (item.tradeWithUserId.toString() === user._id.toString()))} />
                    )}
                  </tradeContext.Consumer>
                </div>
            </div>
