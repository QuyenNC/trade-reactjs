          ...
          case 'chờ xác nhận':
            return (
              <tradeContext.Consumer>
                {({onPutStatusTrade}) => (
                  <Space size="middle">
                  <Button type="primary" onClick={onPutStatusTrade(record._id,'đã hủy')}>
                    Hủy yêu cầu
                  </Button>
                </Space>
                )}
          ...
          case 'đồng ý':
            return (
              <tradeContext.Consumer>
                {({onPutStatusTrade}) => (
                  <Space size="middle">
                    <Button type="primary"  onClick={onPutStatusTrade(record._id,'đang chuyển hàng')}>
                      Đã gửi hàng
                    </Button>
                  </Space>
                )}
          ...
          case 'đang chuyển hàng':
            return (
              <tradeContext.Consumer>
                {({onPutStatusTrade}) => (
                  <Space size="middle">
                    <Button type="primary" onClick={onPutStatusTrade(record._id,'đã nhận hàng')}>
                      Đã nhận được hàng
                    </Button>
                  </Space>
                )}
    ...
    return (
        <div className="RequestExchange">
            <div className="wrap">
                ...
                <tradeContext.Consumer>
                    {({tradeRequest}) => (
                      <Table size='middle' columns={columns} pagination={{ pageSize: 7}} dataSource={tradeRequest.filter(item => (item.userId.toString() === user._id.toString()))} />
                    )}
                  </tradeContext.Consumer>
                </div>
            </div>
