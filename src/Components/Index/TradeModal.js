...
const TradeModal = (props) => {
  const {item, isModalVisible, handleCancel, handleOk, handleChange, inputs, isTrade} = props;
  ...
  return (
    <>
      <Modal  visible={isModalVisible} onOk={handleOk(item)} onCancel={handleCancel} title='Yêu cầu trao đổi' >
      ...
      <select style={{ width: '100%', padding:'10px', fontSize:'16px', border:'1px solid #ddd' }} value={inputs.selectPost} name="selectPost" onChange={handleChange}>
        ...
        <postContext.Consumer>
            {
            ({postItems})=>(
                postItems.filter(item => (item.username === username)).map((item, index) => (
